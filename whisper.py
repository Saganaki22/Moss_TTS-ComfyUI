"""Optional Whisper transcription node for MOSS-TTS continuation prefix text."""
...
def _is_punctuation_only_transcript(text: str) -> bool:
...
def _normalize_transcript(text: str) -> str:
    text = str(text or "").strip()
    if _is_punctuation_only_transcript(text):
        logger.warning("Whisper returned punctuation-only text; treating it as no transcript.")
        return ""
    return text
...
def audio_encoders_dir() -> Path:
    try:
        ...
        base = Path(__file__).resolve().parents[2] / "models" / "audio_encoders"
...
def _has_whisper_files(path: Path) -> bool:
    ...
    try:
        return any(
            item.is_file() and item.suffix.lower() in {".safetensors", ".bin", ".pt", ".pth"}
...
def _build_asr_pipeline(model_path: Path, torch_dtype: torch.dtype, device: str):
    import transformers
...
def get_whisper_pipeline(model_name: str, dtype: str, download_if_missing: bool):
    ...
    if cached is not None:
        patcher = getattr(cached, "_mosstts_aimdo_patcher", None)
    ...
    pipe = _build_asr_pipeline(model_path, torch_dtype, pipeline_device)
    if force_aimdo_dynamic:
        try:
            from . import native
    ...
    try:
        ...
        setattr(pipe, "_mosstts_aimdo_patcher", patcher)
        try:
...
def _audio_has_signal(audio_np: np.ndarray) -> bool:
    if audio_np.size == 0:
        return False
    peak = float(np.max(np.abs(audio_np)))
    if not math.isfinite(peak) or peak <= 1.0e-6:
        return False
    return True
...
def transcribe_audio(
    ...
    if not _audio_has_signal(audio_np):
        logger.warning("Whisper input is silent or empty; returning an empty transcript.")
        return ""
    ...
    return _normalize_transcript(str(result))


class MossTTSWhisperTranscribe:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "audio": (
                    "AUDIO",
                    {"tooltip": "Audio to transcribe. Useful for Continue Speech prefix_text."},
                ),
                "model": (
                    whisper_model_choices(),
                    {
                        "default": "whisper-large-v3-turbo (auto-download)",
                        "tooltip": "Whisper ASR model. Turbo is fast and usually accurate enough for prefix transcripts.",
                    },
                ),
                "dtype": (
                ...
                "language": (
                    WHISPER_LANGUAGE_OPTIONS,
                    {"default": "auto", "tooltip": "Audio language. auto detects it; setting it can improve transcript accuracy."},
                ),
                "task": (
                ...
                "chunk_length_s": (
                    "INT",
                    {
                        ...
                        "max": 120,
                        "step": 1,
                        "tooltip": "Whisper chunk length for longer clips. 0 lets Transformers choose.",
                    },
                ),
                "download_if_missing": (
                    "BOOLEAN",
    ...
    FUNCTION = "transcribe"
    CATEGORY = "MOSS-TTS"
    DESCRIPTION = "Transcribe AUDIO with Whisper for MOSS-TTS continuation prefix text."
